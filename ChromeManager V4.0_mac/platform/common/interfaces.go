...
type WindowProvider interface {
	...
	// CloseWindowQuickly 快速关闭窗口(不等待)。
	CloseWindowQuickly(handle WindowHandle) error
...
type SyncProvider interface {
	...
	// ExecuteBatchInput simulates typing batch text via CDP/native
	ExecuteBatchInput(pids []int, text string, delayed bool, overwrite bool)

	// GetActiveTargetID 获取指定进程当前活动的 TargetID
	GetActiveTargetID(pid int) string
}
...
// AppWindowProvider 提供APP自身窗口的Native控制接口
type AppWindowProvider interface {
	// GetAppWindowPosition 获取APP自身窗口位置 (Native Native Logic)
	// hwnd: 窗口句柄(macOS下可能会忽略直接使用PID)
	GetAppWindowPosition(hwnd uintptr) (Rect, error)

	// SetAppWindowPosition 设置APP自身窗口位置 (Native Native Logic)
	// hwnd: 窗口句柄
	SetAppWindowPosition(hwnd uintptr, rect Rect) error
}
