...
export function initTabs() {
    ...
    debugLog('Found tab panes:', tabPanes.length);
    ...
    debugLog('Tabs initialized successfully');
    return true;
}
...
export function switchToTab(tabId, tabIndex) {
    debugLog('Switching to tab:', tabId, 'index:', tabIndex);
    ...
    if (targetPane) {
        ...
        debugLog('Activated tab pane:', tabId);
    } else {
...
function updateTabSelector(tabIndex) {
    ...
    if (selection && labels.length > 0) {
        ...
        debugLog(`Selection moved to: ${leftPosition}%`);
    }
}
