...
func configuredCacheDir(path string) string {
	return normalizeUserDataDir(strings.TrimSpace(path))
}

func resolveDefaultConfiguredCacheDir(settings *config.Settings) string {
	if settings == nil {
		return ""
	}

	if cacheDir := configuredCacheDir(settings.CacheDir); cacheDir != "" {
		return cacheDir
	}
	...
	for _, group := range settings.Groups {
		...
		if cacheDir := configuredCacheDir(group.CacheDir); cacheDir != "" {
			return cacheDir
		}
	}

	currentGroupName := strings.TrimSpace(settings.CurrentGroup)
	if currentGroupName != "" && currentGroupName != "默认分组" {
		for _, group := range settings.Groups {
			if strings.TrimSpace(group.Name) != currentGroupName {
				continue
			}
...
func (c *ChromeService) resolveGroupCacheDir(groupName string) (string, error) {
	...
	if trimmedName == "" || trimmedName == "默认分组" {
		cacheDir := resolveDefaultConfiguredCacheDir(settings)
		if cacheDir == "" {
			return "", fmt.Errorf("默认分组未配置缓存目录，请先在设置中填写缓存目录或切换到已配置缓存目录的分组")
		}
		return cacheDir, nil
	}
	...
	for _, group := range settings.Groups {
		...
		cacheDir := configuredCacheDir(group.CacheDir)
		if cacheDir == "" {
